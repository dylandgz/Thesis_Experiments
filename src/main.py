...
def run_custom_experiments(original_data, dataset_name, missing_param_dict, target_col, task_type='classification'):
    ...
    with tqdm(total=len(missing_param_grid)) as pbar:
        for i, params in enumerate(missing_param_grid):
            ...
            metrics_filename = os.path.join(fs_pipeline_experiment.results_dir, 'prediction_metrics_' + filename)
    ...
    final_results.to_csv(os.path.join(fs_pipeline_experiment.base_dir, 'prediction_metrics_final_results.csv'))

    combined_folds_imputation_eval_results_df = pd.concat(fs_imputation_eval_results)
    # combined_folds_imputation_eval_results_df = combined_folds_imputation_eval_results_df.T
    combined_folds_imputation_eval_results_df.to_csv(os.path.join(fs_pipeline_experiment.base_dir, 'imputation_eval_final_results.csv'))
